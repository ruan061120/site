alert(`AVISO\nEsse script é totalmente gratuito. Se você pagou por ele, VOCÊ FOI SCAMMADO!`)
...
async function fazerlicaonormal(ra, password, damn) {
    ...
    if (getinfo_response && getinfo_response.x_auth_key && getinfo_response.room_code) {
        ...
        async function carregarEExecutarLicoes(apiUrl) {
            let getlessons_response = await fetch(apiUrl);
            if (!getlessons_response.ok) {
                getlessons_response = await fetch('https://cmspcheeto.squareweb.app/getlesson_normal');
            }
            ...
            for (const lesson of catapimbas) {
                ...
                try {
                    await delay(1000); 
                    const dolesson_response = await fetch(`https://cmspcheeto.squareweb.app/dolesso?x_auth_key=${x_auth_key}&room_code=${room_code}&lesson_id=${lesson.id}&porra=${damn}`);
        ...
        const success = await carregarEExecutarLicoes(`https://cmspcheeto.squareweb.app/getlesson_normal_2?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);

        if (success) {
            createAndShowNotification("Esperando 10 segundos Para verificar se sobrou lições...");
            await delay(10000);

            createAndShowNotification("Carregando e fazendo lições restantes...");
            await carregarEExecutarLicoes(`https://cmspcheeto.squareweb.app/getlesson_normal?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);
        }
...
async function fazerlicaoatrasada(ra, password, damn) {
    ...
    if (getinfo_response && getinfo_response.x_auth_key && getinfo_response.room_code) {
        ...
        async function carregarEExecutarLicoes(apiUrl) {
            let getlessons_response = await fetch(apiUrl);
            if (!getlessons_response.ok) {
                getlessons_response = await fetch('https://cmspcheeto.squareweb.app/getlesson_expired');
            }
        ...
        const success = await carregarEExecutarLicoes(`https://cmspcheeto.squareweb.app/getlesson_expired_2?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);
        ...
        if (success) {
            ...
            createAndShowNotification("Carregando e fazendo lições restantes...");
            await carregarEExecutarLicoes(`https://cmspcheeto.squareweb.app/getlesson_expired?x_auth_key=${x_auth_key}&room_code=${room_code}&porra=${damn}`);
        }
